...
public class Rectangle {
	...
	public int[][] levelOutCourt() {		
		...
		for (int x=0;x<512;x++) for (int y=0;y<512;y++) {
			...
			System.out.println("("+x+","+y+"): "+beforePixel+"-"+afterPixel+" >>> sum="+cost);
		}
	...
	public void setPixelAltitude(int startImageX, int startImageY, int desiredAltitude) {
		...
		System.out.println("\n("+startImageX+","+startImageY+") = "+imageArray[startImageY][startImageX]);
		System.out.println("transport dirt out? "+transportingDirtOutOfRectangle);
		...
		if (transportingDirtOutOfRectangle) {
			...
			recArray=getRectangleArray();
			System.out.println("moving dirt from ("+startImageX+","+startImageY+") to ("+newImageX+","+newImageY+")");
			System.out.println("("+startImageX+","+startImageY+") has a value of "+imageArray[startImageY][startImageX]+"; ("+newImageX+","+newImageY+") has a value of "+imageArray[newImageY][newImageX]);
		} else {
			this.imageArray[startImageY][startImageX]++;
			this.imageArray[newImageY][newImageX]--;
			recArray=getRectangleArray();
			System.out.println("^^moving dirt to ("+startImageX+","+startImageY+") from ("+newImageX+","+newImageY+")");
			System.out.println("("+startImageX+","+startImageY+") has a value of "+imageArray[startImageY][startImageX]+"; ("+newImageX+","+newImageY+") has a value of "+imageArray[newImageY][newImageX]);
		}
	}
	...
	public void check() {
		...
		for (int i=0;i<256;i++) {
			if (altitudes[i]==2048) System.out.println("RECTANGLE IS UNIFORM FUCK YEAH");
	...
	public int getMostFrequentAltitude() {
		...
		//print array plz
		for (int i=0;i<256;i++) {
			System.out.println(i+". "+altitudes[i]);
		}
		...
		for (int i=0;i<256;i++) {
			if (numPixels<altitudes[i]) {
				...
				frequentAlt=i;
				System.out.println("most frequent altitude so far is "+frequentAlt+" with "+numPixels+" pixels");
			}
		}
		System.out.println("WINNER WINNER CHICKEN DINNER: "+frequentAlt);
		return frequentAlt;
	}
}
